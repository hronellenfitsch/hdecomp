...
def horton_strahler_index(tree, root):
    """ Returns the Horton-Strahler index of the tree.
    """
    succ = [horton_strahler_index(tree, s) \
        for s in tree.successors(root)]
...
def shreve_index(tree, root):
    """ Returns the Shreve index of the tree.
    """
    succ = [shreve_index(tree, s) \
        for s in tree.successors(root)]
...
def remove_external_nodes(tree):
    ...
    internal_nodes = [n for n in no_ext_tree.nodes() if
            len(list(no_ext_tree.successors(n))) == 1 \
            and len(list(no_ext_tree.predecessors(n))) == 1]

    for i in internal_nodes:
        pr = list(no_ext_tree.predecessors(i))[0]
        su = list(no_ext_tree.successors(i))[0]

        no_ext_tree.add_edge(pr, su)
        no_ext_tree.remove_node(i)

    # Handle case of root node
    root_succ = list(no_ext_tree.successors)(root)

    if len(root_succ) == 1:
        s = root_succ[0]
        su = list(no_ext_tree.successors(s))
...
def low_level_avg_asymmetries(tree, degree, Delta,
    ...
    roots = [n for n in tree_new.nodes_iter()
            if len(list(tree_new.predecessors(n))) == 0 and
            len(list(tree_new.successors(n))) == 2]
...
def analyze_tree(tree):
    # calculate metrics
    print("Calculating Horton-Strahler index (set to zero..).")
    horton_strahler = 0#horton_strahler_index(tree, tree.graph['root'])
    print("Calculating Shreve index.")
    shreve = shreve_index(tree, tree.graph['root'])
...
if __name__ == '__main__':
    ...
    # print info
    print("Horton-Strahler index:", horton_strahler)
    print("Shreve index:", shreve)
    print("Asymmetry:", tree_asymmetry)
    print("Asymmetry without external nodes:", tree_asymmetry_no_ext)
