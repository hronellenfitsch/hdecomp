...
class Cycle():
    ...
    def __init__(self, graph, edges, coords=None):
        ...
        if coords is None:
            self.coords = array([ graph.node[n]['pos']  #[graph.node[n]['x'], graph.node[n]['y']]
                    for n in self.path])
        else:
            self.coords = coords
        self.edges = edges
    ...
    def ordered_edges(self, edges):
        ...
        con = (edge_graph.subgraph(c) for c in nx.connected_components(edge_graph))
        ...
        for comp in con:
            ...
            component_edges = list(comp.edges())
            ...
            # coordinates of path
            coords = array([ G.node[u]['pos']   #(G.node[u]['x'], G.node[u]['y'])
                    for u, v in sorted_edges] \
                            + [G.node[sorted_edges[0][0]]['pos']])
    ...
    def length(self):
        ...
        """
        return array([self.graph[u][v]['weight']
            for u, v in self.edgeset]).sum()
...
def traverse_graph(G, start, nextn):
    ...
    """
    # print(G.node[start])
    start_coords = G.node[start]['pos']#array([G.node[start]['x'], G.node[start]['y']])
    nodes_visited = [start]
    ...
    while cur != start:
        cur_coords = G.node[cur]['pos']#array([G.node[cur]['x'], G.node[cur]['y']])
        ...
        if n_neighs > 1:
            ...
            prev_coords = G.node[prev]['pos']#array([G.node[prev]['x'], G.node[prev]['y']])
            neigh_coords = array([ G.node[n]['pos'] #[G.node[n]['x'], G.node[n]['y']] \
                for n in neighs])
...
def shortest_cycles(G, verbose=False):
    ...
    if len(cycleset) != n_cycles:
        print("WARNING: Found only", len(cycleset), "cycles!! Wanted:", n_cycles)
